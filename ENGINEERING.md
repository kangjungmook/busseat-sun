...
[README](README.md)에서 갈라 나온 상세 기록입니다. API 응답의 실제 모양, 겪은
함정, 그때 왜 그렇게 결정했는지 — 다음 사람이(또는 6개월 뒤의 내가) 같은 데
빠지지 않도록 남깁니다.
...
**1. 릴리스 서명 — gradle 배선은 끝났고, 키스토어만 만들면 됩니다.**

`android/app/build.gradle`이 `android/key.properties`(gitignored)를 읽어
릴리스 서명을 붙입니다. 그 파일이나 키스토어가 없으면 예전처럼 디버그 키로
떨어지되, **빌드할 때 경고를 찍습니다** — 디버그 서명된 산출물을 모르고
콘솔에 올리는 일이 없도록.

`android/key.properties.example`을 복사해 값을 채우세요. 키스토어 생성 명령도
그 파일 주석에 있습니다. 두 가지 함정을 적어뒀습니다: 경로에 **역슬래시를 쓰면
안 되고**(`.properties`에서 이스케이프 문자), 경로·비밀번호에 **한글을 쓰면
안 됩니다**(`Properties.load`가 ISO-8859-1로 읽습니다).

난독화(`minifyEnabled`)는 켜지 않았습니다. 카카오 SDK 쪽에 규칙을 따로 써야
하는데, 실기기 검증도 안 된 상태에서 켜면 **릴리스에서만 깨지는 버그**를
만들게 됩니다.

> ⚠️ **키스토어와 비밀번호를 잃으면 같은 앱을 다시는 업데이트할 수 없습니다.**
> 저장소에 넣지 말고 백업은 따로 두세요.
>
> gradle 변경은 **Groovy 문법 파싱까지만 검증했습니다.** 이 개발 환경에
> 안드로이드 SDK가 없어 실제 빌드는 돌려보지 못했습니다.

**0. 등록 자산과 콘솔 답변은 `store/`에 정리해 뒀습니다.** 아이콘(512)·그래픽
이미지(1024x500)는 `python3 tool/store_assets.py`로 만들고, 앱 이름·설명·데이터
안전 섹션 답변은 `store/LISTING.md`에 있습니다. **스크린샷만 실기기에서 직접
찍어야 합니다** — 웹 미리보기 화면에는 '미리보기' 배너와 '예시' 표시가 들어가서
실제 앱이 아닌 것을 보여주게 됩니다.

**2. 개인정보처리방침 — 페이지는 작성됨, 연락처만 비어 있습니다.**
`web/privacy.html`에 있고, `flutter build web`이 `web/` 아래 파일을 그대로
복사하므로 빌드할 때마다 `docs/privacy.html`로 따라갑니다 (`docs/`는 매번
지웠다 다시 만들기 때문에 원본을 `web/`에 두는 것이 중요합니다).
제출용 주소는 `https://kangjungmook.github.io/busseat-sun/privacy.html`.

**남은 것**: 문의처가 `PUT-YOUR-EMAIL-HERE` 자리표시자입니다. 구글 플레이는
연락 수단이 있어야 심사를 통과시키는데, 공개 페이지에 그대로 노출되므로 개인
주소를 쓸지 별도 주소를 만들지는 결정이 필요합니다. 데이터 안전(Data safety)
섹션도 콘솔에서 따로 채워야 합니다 — 방침 내용과 어긋나면 반려됩니다.

방침에 적은 내용은 코드에서 확인한 사실입니다: 자체 서버 없음, 분석·광고 SDK
없음(`pubspec.yaml` 확인), 위치는 저장하지 않고 계산·조회에만 사용, 카카오
로그인은 회원번호와 닉네임만 받아 메모리에만 보관, 즐겨찾기·노선 캐시는
`shared_preferences`로 기기에만 저장. **앱 동작을 바꾸면 이 페이지도 같이
고쳐야 합니다.**
...
### 화면 크기 — 2026-09-17에 고친 것

**좁은 화면에서 검색 버튼을 누를 수 없었습니다.** 홈 화면이 고정 높이
`Column`이라 넘치는 만큼 그냥 잘렸고, `360x640`(흔한 보급형 안드로이드)에서
키패드 마지막 줄과 CTA가 화면 밖에 있었습니다. 스크롤도 안 됐습니다. 가로모드는
태양 패널까지만 보였습니다.

지금은 `LayoutBuilder`로 높이를 재서, 680dp보다 낮으면 전체를 스크롤시킵니다
(`Expanded`는 스크롤 안에서 못 쓰므로 그때는 가운데 영역에 고정 높이를 줍니다).
넉넉한 화면에서는 예전 레이아웃 그대로 키패드가 바닥에 붙습니다.

**세로 고정**도 같이 걸었습니다 — `SystemChrome.setPreferredOrientations`와
매니페스트 `android:screenOrientation="portrait"`(activity 속성입니다, application
아닙니다). 다만 **targetSdk 36에서는 큰 화면(약 600dp 이상)에서 이 고정이
무시된다고 알려져 있습니다.** 그래서 실질적인 방어는 위의 스크롤 대응입니다.

**`uses-feature`를 선언했습니다.** 권한만 있으면 플레이가 그 하드웨어를 **필수**로
해석해서, 카메라나 GPS가 없는 기기가 설치 대상에서 빠집니다. 이 앱은 둘 다 없어도
동작하므로(AR은 드래그, 위치는 전국 중심 좌표) `required=false`로 명시했습니다.
...
- **정류장 도착 알림** — 설정에서 행을 지웠습니다. 켜도 아무 일이 없었고,
  제대로 만들려면 **백그라운드 위치 권한**이 필요합니다. 그러면 플레이 심사에서
  별도 정당화를 요구받고, 개인정보처리방침의 "백그라운드에서 위치를 받지
  않습니다"도 같이 고쳐야 합니다. 붙일 거라면 그 셋을 한 묶음으로 봐야 합니다.
- **홈 화면 위젯** — `설정 → 위젯 미리보기`는 앱 안의 그림이고, 실제 홈 화면
  위젯이 아닙니다(`home_widget` 같은 패키지 없음). "3초 안에 알려준다"는 앱에
  가장 잘 맞는 기능이지만 Android/iOS 네이티브 작업이 필요합니다.
...
**좌석 지도 하단도 같은 문제였습니다.** "지하·터널 구간은 계산에서 제외했어요 ·
좌석 배치는 N번 표준 차량 기준" — 터널 데이터는 가진 적이 없어 그 범주를 이미
지웠고, **노선별 차량 좌석 배치 데이터도 없습니다.** 그림은 일반적인 24석 배치이고,
지금은 그렇다고 적습니다. 이런 문구는 코드가 아니라서 테스트에 안 걸리고, 화면만
봐서는 진짜와 구분되지 않습니다 — **스크린샷을 찍으려고 화면을 하나씩 열어보다가
발견했습니다.**

**화면 문구가 없는 기능을 주장하고 있었습니다.** 결과 화면 하단이 "기상청 일사
… 기준 · **건물 그림자 반영**"이었고, 설정의 데이터 출처에 기상청 단기예보 ·
서울 TOPIS · 국토지리정보원 수치표고가 적혀 있었습니다. **셋 다 부르지 않습니다.**
코드가 아니라 문구라서 테스트로도 안 잡히고, 화면만 봐서는 진짜와 구분이
안 됩니다. 이 앱의 결론이 나오는 바로 그 화면에서 근거를 지어내면 사용자가 답을
실제보다 더 믿게 됩니다. 지금은 실제로 쓰는 셋(TAGO · 카카오 로컬 · NOAA 계산식)만
적고, 결과 화면은 "직사광 기준(건물 그림자 미반영)"이라고 밝힙니다.
