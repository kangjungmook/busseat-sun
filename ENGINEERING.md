...
- **설정의 `계산 근거 자세히 보기` 스위치가 아무 데도 연결돼 있지 않습니다.**
  `state.switches[2]`를 읽는 코드가 없어서, 켜도 꺼도 화면이 그대로입니다.
  부제도 "태양 고도·**건물 높이** 값을 함께 표시"인데 건물 높이 데이터는 없습니다.
  구현하거나 행을 지우거나 — 제품 결정이 필요해 그대로 뒀습니다.
